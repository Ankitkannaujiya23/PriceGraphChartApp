...
const processResponse = (response) => {
    if (response?.status === 200) {
        ...
        return { isSuccess: false, statusCode: response?.status, message: response?.message };
    }
};
...
return { isSuccess: false, statusCode: error.response?.status, message: "An error occured." }
...
return { isSuccess: false, statusCode: 400, message: "Something went wrong." };
}
};
